...
namespace x86CS.ATADevice
{
    public class HardDisk : ATADrive
    {
        ...
        public override void LoadImage(string filename)
        {
            ...
            UnityManager.ins.Img_stream.Seek(512, SeekOrigin.Begin);
        ...
        private byte[] ReadSector(long sector)
        {
            ...
            UnityManager.ins.Img_stream.Seek((long)((long)header.TableOffset + (blockNumber * 4)), SeekOrigin.Begin);
            blockOffset = Util.SwapByteOrder(reader.ReadUInt32());

            if (blockOffset == 0xffffffff)
                return new byte[512];

            UnityManager.ins.Img_stream.Seek(blockOffset * 512, SeekOrigin.Begin);
            ...
            UnityManager.ins.Img_stream.Seek(sectorInBlock * 512, SeekOrigin.Current);
        ...
        private void WriteSector(long sector, byte[] data)
        {
            ...
            long sectorInBlock;
            byte[] bitmap;
            BinaryWriter writer = new BinaryWriter(UnityManager.ins.Img_stream);
            ...
            if (blockOffset == 0xffffffff)
            {
                ...
                UnityManager.ins.Img_stream.Seek(-512, SeekOrigin.End);
                offsetPosition = UnityManager.ins.Img_stream.Position;
        ...
        private void Write()
        {
            ...
            UnityManager.ins.Img_stream.Flush();
        }
